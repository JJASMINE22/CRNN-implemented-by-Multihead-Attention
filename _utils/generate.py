...
class Generator():
    ...
    def get_train_len(self):
        ...
        len_lines = open(self.training_path, 'rb').readlines().__len__()
    ...
    def get_val_len(self):
        ...
        len_lines = open(self.validate_path, 'rb').readlines().__len__()
        if not len_lines % self.batch_size:
            return len_lines // self.batch_size
        else:
            return len_lines // self.batch_size + 1
    ...
    def generate(self, training=True):
        if training:
            lines = open(self.training_path, 'rb').readlines()
            np.random.shuffle(lines)
        else:
            lines = open(self.validate_path, 'rb').readlines()
            np.random.shuffle(lines)
